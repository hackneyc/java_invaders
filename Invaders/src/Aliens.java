...
public class Aliens {
	...
	public void updatePosition() {
		...
		if (alienSpeedCount++ >= alienSpeed) {
			...
			while (list.hasNext()) {
				...
				} else if (alienDirection == ALIEN_DIRECTION_LEFT) {
			...
			while (list.hasNext()) {
				...
				if (moveDown) {
					a.setY(a.getY() + 16);
				} else if (alienDirection == ALIEN_DIRECTION_RIGHT) {
					a.setX(a.getX() + 16);
				} else if (alienDirection == ALIEN_DIRECTION_LEFT) {
					a.setX(a.getX() - 16);
				}
	...
	public Aliens(Dimension d, Insets insets) {
		...
		for (int row = 0; row < NUM_ROWS; row++) {
			for (int i = 0; i < ALIENS_PER_ROW; i++) {
				alien[row][i] = new Alien(insets.left + (i * 48),
						60 + (40 * row), 0, "alien0" + row + "_", 2, 0, 32, 32,
						d);
